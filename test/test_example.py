...
from rlf.examples.train import IL_METHODS, RL_METHODS, ExampleRunSettings
...
def test_il_methods(method_name):
    ...
    run_settings = ExampleRunSettings(
        f"--alg {method_name} --env-name {env_name} --cuda False --num-env-steps 5e3 --save-interval -1 --eval-interval -1 --traj-load-path rlf/examples/expert_demonstrations/pendulum_test.pt"
    )
