...
def wrap_deepmind(env, episode_life=True, clip_rewards=True, frame_stack=False,
        scale=False, ignore_fire=False, ignore_wrap=False, grayscale=True):
    """Configure environment for DeepMind-style Atari.
    """
    if episode_life:
        env = EpisodicLifeEnv(env)
    if not ignore_fire and 'FIRE' in env.unwrapped.get_action_meanings():
        env = FireResetEnv(env)

    if '-ram-' not in env.spec.id and not ignore_wrap:
