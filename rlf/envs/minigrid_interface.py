...
class DirectionObsWrapper(gym.core.ObservationWrapper):
    ...
    def __init__(self, env, type="slope"):
        ...
        self.observation_space = rutils.reshape_obs_space(
            self.observation_space, (self.observation_space.shape[0] + 1,)
        )
...
class FullFlatGrid(MiniGridWrapper):
    def __init__(self, env, gw_card):
        ...
        high = 1.0
        self.observation_space = gym.spaces.Box(
            shape=ob_shape, low=np.float32(low), high=np.float32(high), dtype=np.float32
        )
...
class MinigridInterface(EnvInterface):
    ...
    def get_add_args(self, parser):
        ...
        parser.add_argument("--gw-card-dirs", action="store_true", default=False)
        parser.add_argument("--gw-goal-info", action="store_true", default=False)
