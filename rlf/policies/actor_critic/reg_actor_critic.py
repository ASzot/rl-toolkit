...
class RegActorCritic(ActorCritic):
    ...
    def get_action(self, state, add_state, hxs, masks, step_info):
        ...
        if not step_info.is_eval:
            ...
            if (cur_step >= self.args.n_rnd_steps) and (
                np.random.rand() >= self.args.rnd_prob
            ):
                # Get the added noise.
                noise = torch.FloatTensor(
                    [ng.sample(cur_step) for ng in self.noise_gens]
                ).to(self.args.device)
                action += noise

                # Multi-dimensional clamp the action to the action space range.
                action = torch.min(
                    torch.max(action, self.ac_low_bound), self.ac_high_bound
                )
            else:
                action = torch.tensor(
