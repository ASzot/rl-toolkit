...
from rlf.rl.distributions import Categorical, DiagGaussian
from rlf.rl.model import (CNNBase, IdentityBase, MLPBase, MLPBasic,
                          PassThroughBase, TwoLayerMlpWithAction,
                          def_mlp_weight_init, reg_mlp_weight_init,
                          weight_init)
...
def is_image_obs(obs_shape):
    if len(obs_shape) == 3:
        ...
        raise NotImplementedError("Observation space is %s" % str(obs_shape))
...
def get_mlp_net_var_out_fn(hidden_sizes):
    ...
    """
    return lambda i_shp, n_out: MLPBase(
        i_shp[0],
        False,
        [*hidden_sizes, n_out],
        weight_init=reg_mlp_weight_init,
...
def def_get_hidden_net(
    input_shape, hidden_size=64, num_layers=2, recurrent=False, **kwargs
):
    if is_image_obs(input_shape):
        return CNNBase(input_shape[0], False, hidden_size, **kwargs)
    else:
        return MLPBasic(
            input_shape[0], hidden_size=hidden_size, num_layers=num_layers, **kwargs
        )
