...
def get_tb_data(
    ...
    for k, dfs in method_dfs.items():
        ...
        if method_name not in method_steps or len(method_steps[method_name]) < n_steps:
...
def plot_from_file(plot_cfg_path):
    with open(plot_cfg_path) as f:
        ...
        for plot_section in plot_settings["plot_sections"]:
            ...
            use_fig_dims = plot_section.get(
                "fig_dims", plot_settings.get("fig_dims", (5, 4))
            )
            if fig is None:
            ...
            plot_const_lines = get_setting(plot_section, "plot_const_lines", defval={})
            for k, v in plot_const_lines.items():
                uniq_step = plot_df["_step"].unique()
                const_df = pd.DataFrame.from_dict(
                    {
                        "_step": uniq_step,
                        plot_key: np.array(v).repeat(len(uniq_step)),
                        "run": "",
                        "method": k,
                    }
                )
                plot_df = pd.concat([plot_df, const_df])
