...
class PPO(OnPolicy):
        ...
        for e in range(self._arg("num_epochs")):
            ...
            for sample in data_generator:
                ...
                if use_clipped_value_loss:
                    ...
                    ).clamp(-self._arg("clip_param"), self._arg("clip_param"))
                    value_losses = (ac_eval["value"] - sample["return"]).pow(2)
                    value_losses_clipped = (value_pred_clipped - sample["return"]).pow(
                        2
                    )
                    value_loss = (
                    ...
                    value_loss = (
                        0.5 * (sample["return"] - ac_eval["value"]).pow(2).mean()
                    )

                loss = (
                    value_loss * self._arg("value_loss_coef")
                    + action_loss
    ...
    def get_add_args(self, parser):
        ...
        parser.add_argument(
            f"--{self.arg_prefix}entropy-coef",
            type=float,
            default=0.01,
