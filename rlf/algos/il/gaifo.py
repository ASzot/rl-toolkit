...
def get_default_discrim(ac_dim, in_shape):
    ...
    return nn.Sequential(
        ConcatLayer(-1),
        nn.Linear(2 * in_shape[0], hidden_dim),
        nn.Tanh(),
            ...
            for expert_batch, agent_batch in zip(expert_sampler, agent_sampler):
                ...
                if self.args.reward_type == "gaifo":
                    expert_loss = F.binary_cross_entropy_with_logits(
                        expert_d, torch.zeros(expert_d.shape).to(d)
                    )
                    agent_loss = F.binary_cross_entropy_with_logits(
                        agent_d, torch.ones(agent_d.shape).to(d)
                    )
                else:
                    expert_loss = F.binary_cross_entropy_with_logits(
                        expert_d, torch.ones(expert_d.shape).to(d)
                    )
                    agent_loss = F.binary_cross_entropy_with_logits(
                        agent_d, torch.zeros(agent_d.shape).to(d)
                    )
